...
#         a, b = quote_idxs[i-1], quote_idxs[i]
...
# # forward declaration
# parse_sql = None  # type: ignore


# def parse_col(toks, i, tables_with_alias, schema, default_tables=None):
#     tok = toks[i]
#     if tok == '*':
#         return i+1, ('*',)
#     if '.' in tok:
...
# def parse_col_unit(toks, i, tables_with_alias, schema, default_tables=None):
#     if toks[i] in AGG_OPS[1:]:
#         agg = toks[i]; i+=1
#         assert toks[i] == '('; i+=1
...
# def parse_val_unit(toks, i, tables_with_alias, schema, default_tables=None):
#     return parse_col_unit(toks, i, tables_with_alias, schema, default_tables)


# def parse_table_unit(toks, i, tables_with_alias, schema):
#     tbl = toks[i]; i+=1
#     if i < len(toks) and toks[i] == 'as': i+=2
#     return i, tbl


# def parse_condition(toks, i, tables_with_alias, schema, default_tables=None):
#     conds = []
#     while i < len(toks):
#         i, vu = parse_val_unit(toks, i, tables_with_alias, schema, default_tables)
#         not_op = False
#         if toks[i] == 'not': not_op = True; i+=1
#         op = toks[i]; i+=1
#         # parse right side
...
# def parse_select(toks, i, tables_with_alias, schema, default_tables=None):
#     assert toks[i] == 'select'; i+=1
#     distinct = False
...
# def parse_from(toks, i, tables_with_alias, schema):
#     assert toks[i] == 'from'; i+=1
#     tbls, conds = [], []
...
# def parse_group_by(toks, i, tables_with_alias, schema, default_tables=None):
#     if toks[i] != 'group': return i, []
#     i+=2  # skip 'group by'
#     cols = []
#     while i < len(toks) and toks[i] not in CLAUSE_KEYWORDS:
#         i, cu = parse_col_unit(toks, i, tables_with_alias, schema, default_tables)
#         cols.append(cu)
#         if toks[i] == ',': i+=1
#         else: break
#     return i, cols
...
# def parse_order_by(toks, i, tables_with_alias, schema, default_tables=None):
#     if toks[i] != 'order': return i, ('asc', [])
#     i+=2  # skip 'order by'
#     vals = []
#     dir = 'asc'
#     while i < len(toks) and toks[i] not in CLAUSE_KEYWORDS:
#         i, vu = parse_val_unit(toks, i, tables_with_alias, schema, default_tables)
#         vals.append(vu)
#         if toks[i] in ORDER_OPS: dir = toks[i]; i+=1
#         if toks[i] == ',': i+=1
#         else: break
#     return i, (dir, vals)


# def parse_limit(toks, i):
#     if toks[i] == 'limit':
#         val = int(toks[i+1]); i+=2
#         return i, val
#     return i, None


# def parse_sql(toks, i, tables_with_alias, schema):  # type: ignore
#     sql: Dict[str, Any] = {}
#     # SELECT
#     i_sel, sel = parse_select(toks, i, tables_with_alias, schema, [])
#     sql['select'] = sel
#     # FROM
#     j = toks.index('from', i_sel)
#     i_fr, tbls, fr_conds = parse_from(toks, j, tables_with_alias, schema)
#     sql['from'] = {'table_units': tbls, 'conds': fr_conds}
#     # WHERE
#     i_wh, wh = (parse_condition(toks, i_fr, tables_with_alias, schema, tbls)
#                 if i_fr < len(toks) and toks[i_fr] == 'where' else (i_fr, []))
#     sql['where'] = wh
#     # GROUP BY
#     i_gb, gb = parse_group_by(toks, i_wh, tables_with_alias, schema, tbls)
#     sql['groupBy'] = gb
#     # HAVING
#     i_hv, hv = (parse_condition(toks, i_gb, tables_with_alias, schema, tbls)
#                 if i_gb < len(toks) and toks[i_gb] == 'having' else (i_gb, []))
#     sql['having'] = hv
#     # ORDER BY
#     i_ob, ob = parse_order_by(toks, i_hv, tables_with_alias, schema, tbls)
#     sql['orderBy'] = ob
#     # LIMIT
#     i_lm, lm = parse_limit(toks, i_ob)
#     sql['limit'] = lm
#     # I/U/E
#     for op in SQL_OPS: sql[op] = None
#     return i_lm, sql

# parse_sql = parse_sql  # fix forward
...
#         ob = s['orderBy']
#         if ob and ob[1]: comps['orderBy'].append(ob)
...
#     """Evaluator using recursive parse_sql structures"""
#     def __call__(self,  gold_sql: str, pred_sql: str) -> Tuple[float, Dict[str, bool]]:
#         # parse
...
# Usage:
# e = ComponentMatchingEvaluator()
# em, detail = e(pred_sql, gold_sql)
...
# def tokenize(string: str) -> List[str]:
#     s = string.replace("'", '"')
#     # replace CAST(col AS REAL) with just col to simplify parsing
...
# def parse_val_unit(toks, i, tables_with_alias, schema, default_tables=None):
#     # support nested expressions with operators
#     stack = []
#     while i < len(toks):
#         if toks[i] in UNIT_OPS:
#             stack.append(toks[i])
#             i += 1
#         elif toks[i] in AGG_OPS[1:]:
#             agg = toks[i]; i+=1
#             assert toks[i] == '('; i+=1
#             i, col = parse_val_unit(toks, i, tables_with_alias, schema, default_tables)
#             assert toks[i] == ')'; i+=1
#             stack.append((agg, col))
#         elif toks[i] == '(':
#             i+=1
...
#             break
#     return i, tuple(stack) if len(stack) > 1 else stack[0]

# def parse_col_unit(toks, i, tables_with_alias, schema, default_tables=None):
#     return parse_val_unit(toks, i, tables_with_alias, schema, default_tables)
...
#             i, val = parse_sql(toks, i, tables_with_alias, schema)
#         else:
#             val = toks[i] if i < len(toks) else None
#             i += 1
